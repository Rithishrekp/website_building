...
export const updateUserProfile = async (req, res) => {
    ...
    try {
        // Build the update query dynamically to avoid overwriting with nulls if some fields aren't provided
        ...
        if (email) {
            queryStr += `email = $${paramCount}, `;
            queryParams.push(email);
            paramCount++;
        }
        if (phone !== undefined) {
            queryStr += `phone = $${paramCount}, `;
            queryParams.push(phone);
            paramCount++;
        }
        if (bio !== undefined) {
            queryStr += `bio = $${paramCount}, `;
            queryParams.push(bio);
            paramCount++;
        }
        if (avatar_url !== undefined) {
            queryStr += `avatar_url = $${paramCount}, `;
        ...
        queryStr += ` WHERE id = $${paramCount} RETURNING id, name, email, phone, bio, avatar_url, role, is_available, is_onboarded`;
        queryParams.push(id);
